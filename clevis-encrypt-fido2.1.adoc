...
* *rp_id* (string) :
  The relying party id of the credential (that will be created or is provided via
  the "cred_id" field).
...
* *resident* (boolean) :
  Whether or not to create a resident (discoverable) credential. Only used when a new
  credential is generated, i.e. when "cred_id" is not set. A resident credential is
  also listed by `fido2-token -L -r`, which can be useful for portability/backup
  purposes. Decryption behaviour is unchanged either way: it always uses the stored
  "cred_id", never on-device discovery.
  Default: false.
...
* *aaguid* (string) :
  A 32-character hex AAGUID. When set (and "device"/FIDO2_TOKEN is not), instead of
  simply picking the first connected token, the first connected token whose AAGUID
  (as reported by `fido2-token -I`) matches will be used, for both encryption and
  every future decryption. Dashes/spaces are ignored.
  Default: ""
...
link:clevis-fido2-regen.1.adoc[*clevis-fido2-regen*(1)]
